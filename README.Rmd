---
title: "Simulation of trajectories for variable order fractional diffusion"
...
Given a vector of times `endTimes`, this function returns the location 
of one CTRW trajectory at these times (first column) and the number of steps taken 
in the time interval $(0, \text{endTime})$. 
...
`density()` from the `stats` package and plot the densities: 

```{r plot-together, echo=FALSE}
par(mfrow=c(1,2))
plot(density(ensemble1[,1,1]), xlim = c(-2,6), ylim = c(0,0.85), type = 'n', 
     main = "Timescale = 1", xlab='x')
legend("topright", paste(tEnd), col=1:length(tEnd), lty=1, title="Density at t = ")
for (k in 1:length(tEnd)){
    lines(density(ensemble1[,k,1]), col = k)
}
plot(density(ensemble2[,1,1]), xlim = c(-2,6), ylim = c(0,0.85), type = 'n',
     main = "Timescale = 10", xlab='x')
legend("topright", paste(tEnd * 10), col=1:length(tEnd), lty=1, title="Density at t = ")
for (k in 1:length(tEnd)){
    lines(density(ensemble2[,k,1]), col = k)
}
```
